...
import { 
  BookOpen, Calendar, ChevronRight, Search, 
  Trash2, Loader2, ArrowLeft, PlusCircle, LogOut
} from 'lucide-react';
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center font-sans">
        ...
        <span className="font-black text-lg tracking-tight text-slate-500">Accessing Library...</span>
      </div>
    );
  }
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-12">
          <div className="flex items-center space-x-6">
            <Link href="/" className="flex items-center space-x-2 text-slate-500 hover:text-blue-600 font-bold transition-all group">
              <ArrowLeft size={20} className="group-hover:-translate-x-1 transition-transform" />
              <span>Back</span>
            </Link>
            
            <button onClick={handleLogout} className="flex items-center space-x-2 text-slate-400 hover:text-red-500 font-bold transition-all text-sm">
              <LogOut size={16} />
              <span>Sign Out</span>
            </button>
          </div>

          <Link href="/" className="bg-blue-600 text-white px-5 py-2.5 rounded-2xl font-black flex items-center space-x-2 hover:bg-blue-700 transition shadow-lg shadow-blue-100">
            <PlusCircle size={18} />
            <span>New Quiz</span>
          </Link>
        </div>
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-6">
          <div>
            <h1 className="text-5xl font-black tracking-tighter">Your Library</h1>
        ...
        {filteredHistory.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredHistory.map((quiz) => (
              <Link key={quiz.id} href={`/quiz/${quiz.id}`} className="group bg-white p-7 rounded-[32px] border border-slate-100 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all block relative">
          ...
          <div className="text-center py-32 bg-white rounded-[40px] border-2 border-dashed border-slate-200">
             ...
             <p className="text-slate-400 font-black text-xl">No quizzes found.</p>
             <Link href="/" className="mt-4 inline-block bg-slate-900 text-white px-8 py-3 rounded-2xl font-black">
                Start a new session
             </Link>
          </div>
        )}
